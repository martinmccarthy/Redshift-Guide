"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.paginateDescribeDataSharesForConsumer = void 0;
const DescribeDataSharesForConsumerCommand_1 = require("../commands/DescribeDataSharesForConsumerCommand");
const Redshift_1 = require("../Redshift");
const RedshiftClient_1 = require("../RedshiftClient");
const makePagedClientRequest = async (client, input, ...args) => {
    return await client.send(new DescribeDataSharesForConsumerCommand_1.DescribeDataSharesForConsumerCommand(input), ...args);
};
const makePagedRequest = async (client, input, ...args) => {
    return await client.describeDataSharesForConsumer(input, ...args);
};
async function* paginateDescribeDataSharesForConsumer(config, input, ...additionalArguments) {
    let token = config.startingToken || undefined;
    let hasNext = true;
    let page;
    while (hasNext) {
        input.Marker = token;
        input["MaxRecords"] = config.pageSize;
        if (config.client instanceof Redshift_1.Redshift) {
            page = await makePagedRequest(config.client, input, ...additionalArguments);
        }
        else if (config.client instanceof RedshiftClient_1.RedshiftClient) {
            page = await makePagedClientRequest(config.client, input, ...additionalArguments);
        }
        else {
            throw new Error("Invalid client, expected Redshift | RedshiftClient");
        }
        yield page;
        const prevToken = token;
        token = page.Marker;
        hasNext = !!(token && (!config.stopOnSameToken || token !== prevToken));
    }
    return undefined;
}
exports.paginateDescribeDataSharesForConsumer = paginateDescribeDataSharesForConsumer;
